...
#
# Scoped PER RULE, not globally: only the PROSE-shaped rules (internal-marker,
# private-repo-ops) accept it, because only those can fire on a sentence that
# merely discusses the gate. A credential-shaped VALUE is a leak no matter what
# else shares its line — "public-repo-guard caught AKIA…" still contains the key —
# so the credential and infrastructure rules ignore this list and only the
# explicit, visible `guard:allow <reason>` can exempt them.
...
# check <BLOCK|WARN> <name> <regex> <why> [about-ok]
#   Pass "about-ok" as the 5th arg to additionally exempt lines matching
#   ABOUT_THE_CONTROL. Only prose-shaped rules should opt in (see above).
check() {
...
#
# Case-insensitive via (?i:...) scoped to the marker alternation only: these
# markers most often arrive as sentence-initial capitals or shouty headers
# ("INTERNAL ONLY", "Do not share…"), and a case-sensitive match missed exactly
# those. The scoping keeps OPS_DETAIL's deliberate SCREAMING_CASE requirement
# below untouched.
check BLOCK internal-marker  '(?<![“"'"'"'`])\b(?i:internal[- ]only|do\s+not\s+(?:share|publish|distribute)|for\s+internal\s+use)\b(?![”"'"'"'`])' 'Text self-identifies as not-for-public' about-ok
